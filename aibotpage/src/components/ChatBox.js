...
const ChatBox = () => {
  ...
  const chats = [
    ...previousChats.filter(c => c.id !== currentChatId), // prevent duplicates
    { id: currentChatId, date: new Date().toDateString(), messages, rating, feedback: comment }
  ];
  setPreviousChats(chats);

  // then open modal (optional)
